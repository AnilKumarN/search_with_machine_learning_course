...
def transform_query(user_query):
    ...
    translator = user_query.maketrans(string.punctuation, ' '*len(string.punctuation))
    user_query = user_query.translate(translator)
...
def get_query_category(user_query, query_class_model):
    ...
    output_categories = []
    predicted_categories = model_output[0]
    scores = model_output[1]
    score_sum = 0.0
    for i in range(len(predicted_categories)):
        category = predicted_categories[i].replace("__label__", "")
